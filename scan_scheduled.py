...
def fetch_vulnerability_details(input_file: str):
    ...
    output_file = os.path.join(OUTPUT_DIR, "exploitCVE_details_scheduled.txt")
    try:
...
def run_website_scanner(website):
    ...
    output_file = os.path.join(OUTPUT_DIR, "webappAUDIT_scheduled.txt")
    try:
...
def nmap_task(target):
    ...
    nmap_output_file = os.path.join(OUTPUT_DIR, "exploitCVE_scheduled.txt")
    try:
