...
def schedule_script(script_path, time_str, script_arg):
    ...
    cron.remove_all(comment='Scheduled by scheduler script')
    
    # Build the command:
    # Change to the script's directory and then execute the script with the current Python interpreter,
    # passing the additional argument. Paths and argument are quoted to handle spaces/special characters.
    command = f'cd "{script_dir}" && "{sys.executable}" "{script_abs}" "{script_arg}"'
    
    # Create a new cron job with a comment for easy identification
    job = cron.new(command=command, comment='Scheduled by scheduler script')
    ...
    cron.write()
    print(f"Scheduled {script_abs} to run daily at {time_str} with argument '{script_arg}' in its directory {script_dir}")
...
def get_arguments():
    ...
    parser.add_argument('script_arg', help='Additional argument to pass into the scheduled script.')
    args = parser.parse_args()
    return args.script_path, args.time, args.script_arg

if __name__ == '__main__':
    script_path, time_str, script_arg = get_arguments()
    schedule_script(script_path, time_str, script_arg)
